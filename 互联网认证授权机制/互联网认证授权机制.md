...
sequenceDiagram
    ...
    ClientBackend->>Browser: 3. 重定向到授权服务（带client_id/redirect_uri）
...
1. **请求阶段**：
   ...
   - 将`state`与授权请求一起发送到授权服务
   - 客户端应在本地(如session或cookie)存储此`state`值

2. **回调阶段**：
   - 授权服务在重定向URI中返回相同的`state`值
...
sequenceDiagram
    ...
    participant 授权服务 as OAuth2授权服务(auth-server.com)

    用户->>客户端应用: 点击登录按钮
    客户端应用->>浏览器: 生成随机state=xyz123并存储会话中
    浏览器->>授权服务: 跳转授权请求:<br/>/authorize?client_id=123&state=xyz123&...
    Note right of 浏览器: state参数随请求发送
    用户->>授权服务: 认证并授权
    授权服务-->>浏览器: 302重定向到回调地址:<br/>client-app.com/callback?code=AUTH_CODE&state=xyz123
    浏览器->>客户端应用: 发送授权码和state
    客户端应用->>客户端应用: 校验state匹配会话存储值
    alt state验证通过
        客户端应用->>授权服务: 用code换取token（安全继续流程）
    else state验证失败
        客户端应用->>浏览器: 终止流程，返回错误
    end
```
