...
oAuth2采用两种方法防御CSRF攻击：

1. 在用户登录授权服务时，会写好允许的回调地址列表。在同意授权时，授权服务会检查回调地址是否在允许的地址列表中。因此伪造请求无法让授权服务将授权码回调给恶意链接。
2. 在OAuth2授权流程中，客户端会在用户发起登录请求时生成一个随机的state值，并将该值随授权请求一起发送至授权服务器。当授权服务器完成认证后，会在回调请求中原样返回这个state参数。客户端只需验证回调中的state值是否与最初生成的随机值一致，即可确认该授权响应确实是对应于当前会话的合法响应
