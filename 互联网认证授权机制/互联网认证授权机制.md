...
>对于依赖重定向与回调机制的流程（授权码模式、简化模式、PKCE拓展模式），客户端需要提前在授权服务中注册允许的回调地址列表（与`client_id`绑定）
...
1. 客户端需要提前跟授权服务注册一个回调地址列表（和`client_id`绑定）。后续授权请求中携带的回调地址必须在该列表中，因此伪造请求无法让授权服务将授权码回调给恶意链接。
